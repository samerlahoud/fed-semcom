...
# ------------------------------------------------------------------
# DCT-based semantic encoding utilities
# ------------------------------------------------------------------
import numpy as np
...
def simulate_dct_transmission(img, k, snr_db):
    coeffs = dct2(img)
    flat = coeffs.flatten()
    indices = np.argsort(np.abs(flat))[::-1]
    mask = np.zeros_like(flat)
    mask[indices[:k]] = 1
    masked_coeffs = (flat * mask).reshape(coeffs.shape)
    recon = idct2(masked_coeffs)
    snr = 10**(snr_db / 10)
    cost = k / np.log2(1 + snr)
    quality = ssim(img, recon, data_range=1)
    utility = quality - 0.01 * cost
    return quality, cost, utility, recon
